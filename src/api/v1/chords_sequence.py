...
def compose_background_chords(tempo: int, chords: list, quarternotes: int, bassline: bool, percussion: bool, repeat_n_times: Optional[int], timeout: Optional[int], notes_range: tuple) -> str:
    ...
    if percussion:
        midi_composer.add_percussion_part(repeat_n_times*len(chords))
...
@router.post("/chords_sequence", tags=['play_modes'])
def play_background_chords(fields: RequestFieldsChordsSequence, background_tasks: BackgroundTasks):
    """Play chords sequence"""

    output_file_path = compose_background_chords(fields.tempo, fields.chords, fields.quarternotes, fields.bassline, fields.percussion, fields.repeat_n_times, fields.timeout, fields.notes_range)
